...
exports.shinyliveUrlDecode = exports.shinyliveUrlEncode = exports.shinyliveCreateFromExplorer = exports.shinyliveSaveAppFromUrl = exports.shinyliveCreateFromActiveEditor = void 0;
 ...
 * @async
 */
async function shinyliveCreateFromActiveEditor() {
    if (!vscode.window.activeTextEditor) {
        vscode.window.showErrorMessage("Shinylive: no editor is currently active.");
        return;
    }
    const content = vscode.window.activeTextEditor.document.getText();
    const { fileName: filePath, languageId: language, isUntitled, } = vscode.window.activeTextEditor.document;
    if (!language || !["r", "python"].includes(language)) {
        vscode.window.showErrorMessage("Shinylive only supports Python and R apps.");
        return;
    }
    if (!isUntitled) {
        const { name: fileName } = path.parse(filePath);
        if (!/(^app|app$)/i.test(fileName)) {
            vscode.window.showErrorMessage("Shinylive: A single-file Shiny app is required when creating a link from the active editor.");
            return;
        }
    }
    ...
    await createAndOpenShinyliveLink(files, extension.toLowerCase());
}
exports.shinyliveCreateFromActiveEditor = shinyliveCreateFromActiveEditor;
/**
 ...
 */
async function createAndOpenShinyliveLink(files, language = "py") {
 ...
 * @async
 */
async function shinyliveSaveAppFromUrl(url) {
    ...
    if (files.length < 1) {
        vscode.window.showErrorMessage("Shinylive: The provided link did not contain any files.");
        return;
    }
    ...
    const localFiles = await shinyliveWriteFiles(files, outputDir, files.length > 1 // confirmOverwrite: vscode save dialog will have asked already
    );
...
exports.shinyliveSaveAppFromUrl = shinyliveSaveAppFromUrl;
function filesAreNotContainedSingleDir(files) {
    const bad = files.map((f) => f.name).filter((nm) => nm.startsWith(".."));
    if (bad.length) {
        vscode.window.showErrorMessage("Shinylive link includes files that cannot be written into a " +
 ...
 * Explorer.
 */
async function shinyliveCreateFromExplorer(_activatedFile, selectedFiles) {
    const allFiles = [];
    ...
    const allFilesSorted = allFiles
        ...
        .filter((file, index, self) => index === self.findIndex((x) => x.fsPath === file.fsPath))
        // Push shiny app file to the start of the list
        .sort((a, b) => {
        const aIsAppFile = (0, extension_1.isShinyAppUsername)(a.path, "python") || (0, extension_1.isShinyAppUsername)(a.path, "r");
        const bIsAppFile = (0, extension_1.isShinyAppUsername)(b.path, "python") || (0, extension_1.isShinyAppUsername)(b.path, "r");
        // A first (-1) or B first (+1) or same
        return -aIsAppFile + +bIsAppFile;
    });
    const primaryFile = allFilesSorted[0].path;
    const isPythonApp = (0, extension_1.isShinyAppUsername)(primaryFile, "python");
    const isRApp = (0, extension_1.isShinyAppUsername)(primaryFile, "r");
    if (!isPythonApp && !isRApp) {
        vscode.window.showErrorMessage("Shinylive: the selected files did not contain a Shiny for Python or R app.");
        return;
    }
    const rootDir = path.dirname(primaryFile);
    const pathRelativeToRootDir = (file) => path.relative(rootDir, file.path);
    const files = await Promise.all(allFilesSorted.map(async (file) => {
        const type = (0, istextorbinary_1.isBinary)(file.fsPath) ? "binary" : "text";
...
exports.shinyliveCreateFromExplorer = shinyliveCreateFromExplorer;
/**
 ...
 */
async function readDirectoryRecursively(uri) {
 ...
 */
async function askUserForOpenAction() {
    // first check if the user has set a default action
    const prefAction = vscode.workspace
 ...
 * string if the user cancelled.
 */
async function askUserForAppMode() {
    // first check if the user has set a default mode
    const prefMode = vscode.workspace
        .getConfiguration("shiny.shinylive")
        .get("appMode") || "ask";
 ...
 */
async function askUserForOutputFile(defaultName) {
 ...
 */
async function askUserForOutputDirectory() {
    ...
    if (uri.scheme !== "file") {
        vscode.window.showErrorMessage("Shinylive: Remote directories are not supported at this time. " +
    ...
    try {
        ...
        vscode.window.showErrorMessage(`Shinylive failed to create new directory "${uri.toString()}". ${error}`);
        return;
    }
    lastUsedDir = uri;
    return uri;
 ...
 */
function shinyliveUrlEncode({ language, files, mode }) {
...
exports.shinyliveUrlEncode = shinyliveUrlEncode;
/**
 ...
 */
function shinyliveUrlDecode(url) {
    const { hash, pathname } = new URL(url);
    const { searchParams } = new URL("https://shinylive.io/?" + hash.substring(1));
    const code = searchParams.get("code");
    if (!code) {
        return;
    }
    const filesJson = lzstring.decompressFromEncodedURIComponent(code);
    const files = JSON.parse(filesJson).map((file) => {
        file.name = path.normalize(file.name);
        return file;
    });
    const pathParts = pathname.split("/");
    const language = pathParts.includes("py") ? "py" : "r";
    const mode = pathParts.includes("editor") ? "editor" : "app";
    return { language, mode, files: files };
}
exports.shinyliveUrlDecode = shinyliveUrlDecode;
/**
 ...
 * conflicts.
 */
async function shinyliveWriteFiles(files, outputDir, confirmOverwrite = true) {
    const localFiles = [];
    for (const file of files) {
        ...
        if (fileDir.scheme === "file" && !(await pathExists(fileDir))) {
            ...
            try {
                ...
                vscode.window.showErrorMessage(`Shinylive failed to create directory "${fileDir.toString()}". ${error}`);
                return [];
            }
        }
        let contentBuffer;
        ...
        if (!confirmOverwrite || (await askUserToConfirmOverwrite(filePath))) {
            try {
                ...
                vscode.window.showErrorMessage(`Shinylive failed to write file ${file.name} to "${filePath.toString()}". ${error}`);
                return [];
            }
        }
    }
...
async function askUserToConfirmOverwrite(filePath) {
    ...
    const userOverwrite = await vscode.window.showInformationMessage(`File exists, overwrite? "${filePath.fsPath}"`, "Yes", "No");
...
//# sourceMappingURL=shinylive.js.map